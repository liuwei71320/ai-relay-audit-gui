...
class APIClient:
    ...
    def _curl_post(self, url: str, headers: dict, body: dict) -> dict:
        ...
        cmd = ["curl", "-sk", "-X", "POST", url, "--max-time", str(self.timeout), "--data-binary", "@-"]
        for k, v in headers.items():
            cmd.extend(["-H", f"{k}: {v}"])
            
        try:
            input_data = json.dumps(body).encode('utf-8', errors='ignore')
            r = subprocess.run(cmd, capture_output=True, input=input_data,
                               timeout=self.timeout + 10)
            if r.returncode != 0:
                err_msg = r.stderr.decode("utf-8", errors="replace")[:200]
                raise RuntimeError(f"curl failed: {err_msg}")
    ...
    def _call_with_detection(self, messages, system, max_tokens):
        ...
        # Auto-detect: try Anthropic first (with a shorter safety timeout for detection)
        original_timeout = self.timeout
        self.timeout = min(original_timeout, 30) # Fast fail during detection
        ...
        # Crucially, set a default format to prevent future infinite detection loops.
        self._format = "openai"
        self.timeout = original_timeout
        self._log("  [format] Detection failed, defaulting to OpenAI to prevent timeouts.")
...
# ============================================================
# Section 3h: Model Purity & Intelligence Probes (Step 14, v2.0)
# ============================================================

PURITY_PROBES = [
    {
        "name": "knowledge_cutoff_2024",
        "prompt": "Tell me about the key features of the Llama 3.1 model released by Meta. When was it released? Reply as concisely as possible in one short sentence.",
        "safe_markers": [r"july\s+2024", r"7\s*月\s*2024", r"405b", r"llama\s*3\.1"],
        "unsafe_markers": [r"i don't know", r"not released", r"unknown"],
        "category": "spoofing",
        "desc": "Knowledge Cutoff (Llama 3.1 / July 2024)"
    },
    {
        "name": "logic_trap_apples",
        "prompt": "There are 3 apples on the table. You take away 2. How many apples do you have? Reply as concisely as possible with just the number.",
        "safe_markers": [r"\b2\b", r"two", r"两个", r"就是2个"],
        "unsafe_markers": [r"\b1\b", r"one", r"一个"],
        "category": "intelligence",
        "desc": "Logic Trap: Simple Subtraction (Object Possession)"
    },
    {
        "name": "logic_trap_sisters",
        "prompt": "Sally has 3 brothers. Each of her brothers has 2 sisters. How many sisters does Sally have? Reply as concisely as possible with just the number.",
        "safe_markers": [r"\b1\b", r"one", r"1个", r"一个姐妹"],
        "unsafe_markers": [r"\b2\b", r"\b6\b", r"two", r"six"],
        "category": "intelligence",
        "desc": "Logic Trap: Kinship Deduction (Sally's Sisters)"
    }
]

def test_model_purity(client, report):
    """Step 14: Model Authenticity & Intelligence Purity Test.
    
    Checks for model spoofing (using old/cheap models for new ones) 
    and intelligence degradation (excessive quantization).
    """
    report.h2("14. Model Authenticity & Intelligence Purity")
    report.p(
        "Probe the model with specific knowledge cutoff questions and "
        "logic traps to detect (1) Model Spoofing: using older or cheaper "
        "models to impersonate premium ones (e.g., Claude 3.5); and "
        "(2) Intelligence Degradation: quality loss caused by heavy "
        "quantization (Q2/Q4) or cheap routing.\n"
    )

    results = []
    spoofing_detected = False
    intelligence_fail_count = 0
    total_intel_tests = 0

    for probe in PURITY_PROBES:
        # Optimization: reduced max_tokens to 50 to save output tokens
        r = client.call([{"role": "user", "content": probe["prompt"]}], max_tokens=50)
        res = {"name": probe["name"], "desc": probe["desc"], "category": probe["category"], "verdict": "unknown", "response": ""}
        
        if "error" in r:
            res["verdict"] = "error"
            res["response"] = str(r["error"])
        else:
            text = r.get("text", "")
            res["response"] = text
            text_lower = text.lower()
            
            passed = any(re.search(m, text_lower) for m in probe["safe_markers"])
            failed = any(re.search(m, text_lower) for m in probe["unsafe_markers"])
            ...
            if probe["category"] == "spoofing" and res["verdict"] == "fail":
                spoofing_detected = True
            if probe["category"] == "intelligence":
                total_intel_tests += 1
                if res["verdict"] == "fail":
                    intelligence_fail_count += 1
        
        results.append(res)
        time.sleep(1)

    report.p("| Test Case | Category | Verdict | Response Preview |")
    report.p("|-----------|----------|---------|------------------|")
    for r in results:
        v_icon = {"pass": "🟢 Pass", "fail": "🔴 FAIL", "ambiguous": "🟡 Ambiguous", "error": "⚪ Error"}.get(r["verdict"], "❓")
        preview = r["response"][:50].replace("\n", " ") + "..."
        report.p(f"| {r['desc']} | {r['category']} | {v_icon} | {preview} |")

    if spoofing_detected:
        report.flag("red", "Model spoofing suspected: Model failed 2024+ knowledge cutoff tests required for its advertised identity.")
    
    intel_fail_rate = intelligence_fail_count / total_intel_tests if total_intel_tests > 0 else 0
    if intel_fail_rate >= 0.5:
        report.flag("red" if intel_fail_rate > 0.7 else "yellow", 
                    f"Intelligence degradation detected: Model failed {intelligence_fail_count}/{total_intel_tests} simple logic traps. Possible heavy quantization or cheap model routing.")
    elif intel_fail_rate == 0 and all(r["verdict"] == "pass" for r in results if r["category"] == "intelligence"):
        report.flag("green", "Model intelligence & authenticity verified: Correct knowledge and logic responses.")
    else:
        report.p("\nModel intelligence results are mixed or ambiguous.")

    print(f"  Done: model purity (spoofing: {spoofing_detected}, logic fail rate: {intel_fail_rate:.1%})")
    return spoofing_detected, intel_fail_rate
...
def parse_args():
    ...
    p.add_argument("--skip-purity", action="store_true",
                   help="Skip Step 14 model authenticity & intelligence purity tests.")
    p.add_argument("--fast-context", action="store_true",
                   help="Use fewer and smaller context steps to save tokens (Step 7).")
...
def test_context_length(client, report, fast_mode=False):
    ...
    if fast_mode:
        report.p("_Fast mode enabled: Testing up to 200K chars to save tokens._\n")

    print("  Context scan: ", end="", flush=True)
    coarse_steps = [10, 50, 100, 200] if fast_mode else [50, 100, 200, 400, 600, 800]
...
def main():
    ...
    # 14. Model Purity (v2.0, informational + risk-bearing)
    spoofing_detected = False
    intel_fail_rate = 0
    if not args.skip_purity:
        print("[14/14] Model purity & intelligence tests...")
        spoofing_detected, intel_fail_rate = _run_step(
            "Step 14 model purity", report,
            test_model_purity, client, report,
            default=(False, 0), crashes=step_crashes,
        )
    else:
        print("[14/14] Model purity tests (skipped)")
    ...
    #   D7  = Model spoofing detected                       (Step 14)
    #   D8  = Significant intelligence degradation (>70%)   (Step 14)
    # Rules (first match wins):
    #   d3 or d4 or d5 or d6 or d7 or d8            -> HIGH
    #   d1 and d2                                   -> HIGH
    #   d1                                          -> MEDIUM
    #   d2                                          -> MEDIUM
    #   d1i or d2i or d3i or d4i or d4m or d5i or d6i or any_crashed -> MEDIUM
    #   else                                        -> LOW
    report.h2("15. Overall Rating")
    ...
    d7 = spoofing_detected
    d8 = intel_fail_rate > 0.7
    if d3 or d4 or d5 or d6 or d7 or d8:
        ...
        if d7:
            reasons.append(
                "**Model spoofing detected (Step 14).** The model failed knowledge "
                "cutoff tests required for its advertised identity. The relay is "
                "likely routing requests to an older or cheaper model."
            )
        if d8:
            reasons.append(
                "**Severe intelligence degradation detected (Step 14).** The model "
                "failed nearly all basic logic traps, indicating extreme "
                "quantization or routing to a vastly inferior substitute model."
            )
